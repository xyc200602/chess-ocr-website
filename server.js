...
class GLMChessRecognizer {
  constructor(apiKey = null) {
    ...
    this.model = process.env.ZHIPU_MODEL || 'glm-4.6v';  // Updated to glm-4.6v
  ...
  try {
    ...
    if (method === 'glm') {
      ...
      if (!apiKey && !process.env.ZHIPU_API_KEY) {
        ...
        return res.status(400).json({
          success: false,
          error: 'API key is required for GLM-4.6V recognition. Please provide it in the frontend input or configure it in .env file.'
        });
      }
