...
- **🤖 GLM-4V AI Recognition** (Recommended)
  - Uses Zhipu AI's GLM-4V vision-language model
...
7. **Manual Correction** (if needed)
   - Edit moves in the input field
...
- Try GLM-4V for better results

**Problem**: Slow processing
- First run downloads language pack
- Switch to GLM-4V (faster & more accurate)
...
一个基于智谱GLM-4V视觉大模型和OCR技术的国际象棋手写棋谱识别网站，可以将手写或打印的棋谱图片自动转换为标准的PGN格式。
...
> **注意**: 如果不使用GLM-4V，可以跳过此步骤，直接使用Tesseract OCR识别。
...
2. **选择识别方式**
   - **GLM-4V 智能识别**（推荐）：适合手写和复杂棋谱，需要配置API密钥
...
- **错误**: `GLM-4V API Error: Invalid API key`
  - 解决：检查API密钥是否正确
  - 确保密钥有效且有足够的额度
...
- ✅ **集成智谱GLM-4V视觉大模型**（识别准确度大幅提升）
- ✅ 双识别引擎设计（GLM-4V + Tesseract OCR）
- ✅ 智能fallback机制（GLM-4V失败自动切换到OCR）
